...
/*   Updated: 2024/09/28 21:10:30 by castronela       ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_op(t_lst_str *node_current, t_lst_str *node_prev);
...
static int	check_op(t_lst_str *node_current, t_lst_str *node_prev)
{
	if (is_op_redir(node_current->str, NULL))
	{
		...
		else if (is_op_redir(node_current->next->str, NULL)
			|| is_op_ctrl(node_current->next->str, NULL))
		{
			printf("%s `%s\'\n", ERR_STX_UNEXP_TOKEN, node_current->next->str);
			return (1);
		}
	}
	if (is_op_ctrl(node_current->str, NULL) && (!node_prev
			|| !node_current->next))
	{
